...
def find_all_paths(maze, start, end):
   ...
   -------
   paths : list 
       Restituisce tutti i percorsi trovati tra partenza e arrivo.
   """
    ...
    while stack:
        # Prendiamo il primo percorso dalla pila, poiché la pila è una lista di liste, il primo elemento sarà l'intero percorso,
        # per cui il percorso viene salvato nella variabile path e queue diventa una variabile deque contenente una lista vuota
        path = stack.pop()
        ...
        # Altrimenti, esploriamo le posizioni adiacenti
            ...
            for next_pos in get_adjacent_positions(maze, curr_pos):
                ...
                if next_pos not in path:
                    # Creiamo un nuovo percorso che include la posizione adiacente e lo inseriamo come nuova pila
                    new_path = list(path)
...
def get_adjacent_positions(maze, pos):
    ...
    for i,j in adjacent_positions:
        # Controlliamo che non stiamo uscendo dal labirinto e che non sia un muro
